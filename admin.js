/* 준스키타운 관리자 페이지 로직 (Admin.dc.html 디자인 구현) */
(function () {
  'use strict';

  // ── 로그인 세션 확인 ─────────────────────────────────────────
  try {
    if (!localStorage.getItem('jst_admin_session')) {
  ...
  var state = {
    activeTab: 'lift',
    catalog: load('jst_catalog', defaultCatalog(), function (p) {
      return p && p.lift && p.equipment && p.clothing && p.safety;
    }),
    discountConfig: mergeDiscountConfig(load('jst_discount_config', null)),
    notices: load('jst_notices', defaultNotices(), Array.isArray),
    inquiries: load('jst_inquiries', defaultInquiries(), Array.isArray),
  ...
  var persistCatalog = function () { persist('jst_catalog', state.catalog); };
  var persistDiscount = function () { persist('jst_discount_config', state.discountConfig); };
  var persistNotices = function () { persist('jst_notices', state.notices); };
  var persistInquiries = function () { persist('jst_inquiries', state.inquiries); };
  ...
  document.getElementById('logoutBtn').addEventListener('click', function () {
    try { localStorage.removeItem('jst_admin_session'); } catch (e) {}
